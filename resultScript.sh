...
# # Loop through the associative array and print its contents
...
for key in "${!metric_data[@]}"; do
    metric="${key%%-*}"  # Extract the metric part
    value="${metric_data[$key]}"  # Get the value
    # Check if the key ends with "-[0-9]" using a regular expression
    if [[ "$key" =~ -[0-9]$ ]]; then
        # Append the values for the same metric to the combined_metrics array
        combined_metrics["$metric-p"]+=" $key:$value"
    else
...
echo "\"${combined_metrics["f1-p"]}\",\"${combined_metrics["precision-p"]}\",\"${combined_metrics["recall-p"]}\",\"${combined_metrics["acc-p"]}\",\"${combined_metrics["f1-all"]}\",\"${combined_metrics["precision-all"]}\",\"${combined_metrics["recall-all"]}\",\"${combined_metrics["acc-all"]}\",\"${combined_metrics["final-f1"]}\",\"${combined_metrics["final-precision"]}\",\"${combined_metrics["final-recall"]}\",\"${combined_metrics["final-acc"]}\"" >> "$output_csv"
